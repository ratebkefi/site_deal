    ...
    public function addAction($user_id)
    {
        ...
        if ($request->getMethod() == 'POST') {
            ...
            if ($form->isValid()) {
                ...
                $libelleNotif2 = $agent . " vous a ajouté un nouveau contrat";
                $lient = $this->generateUrl('user_contract_show', array('user_id'=>$contract->getUser()->getId(),'id'=>$contract->getId()));
                //$lient = "user_contract_show, { 'user_id': " . $contract->getUser()->getId() . " , 'id':".$contract->getId()." }";
