...
class PlannerFilterType extends AbstractType
{
    ...
    {
        $builder
            ->add('name', 'entity', array('label' => "Nom", 'empty_value' => 'Choisissez un plannificateur',
                'class' => 'UserUserBundle:User',
                'query_builder'=> function(UserRepository $r) {
                    return  $r->createQueryBuilder("u")
                        ->where('u.roles LIKE :roles')
                        ->setParameter('roles', '%"PALAINER"%');
                },'property' => 'name',  'multiple' => false,'required' => false,))
            ->add('email', 'entity', array('label' => "E-mail", 'empty_value' => 'Choisissez email plannificateur',
                'class' => 'UserUserBundle:User',
                'query_builder'=> function(UserRepository $r) {
                    return  $r->createQueryBuilder("u")
                        ->where('u.roles LIKE :roles')
    ...
    {
        return 'back_commandebundle_plannerfilter';
    }
}
